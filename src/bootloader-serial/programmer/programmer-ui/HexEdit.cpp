...
{
	unsigned int off = m_nBaseAddress + nOffset;

	*sBuffer++ = szHexDigits[(off >> (4 * 7)) & 0xf];
	*sBuffer++ = szHexDigits[(off >> (4 * 6)) & 0xf];
	*sBuffer++ = szHexDigits[(off >> (4 * 5)) & 0xf];
	*sBuffer++ = szHexDigits[(off >> (4 * 4)) & 0xf];
	*sBuffer++ = szHexDigits[(off >> (4 * 3)) & 0xf];
	*sBuffer++ = szHexDigits[(off >> (4 * 2)) & 0xf];
	*sBuffer++ = szHexDigits[(off >> (4 * 1)) & 0xf];
	*sBuffer++ = szHexDigits[(off) & 0xf];
}
